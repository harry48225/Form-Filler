...
import com.harry.formfiller.user.QuestionStat;
...
public class FormDisplayer extends JFrame implements ActionListener, MouseListener, WindowListener
{
	/* This is the main class that allows the user to attempt a form.
		...
		allows them to fill it in. It also tracks all of the statistics
		about the user's attempt. */
		
		
	private Form form;
	private JPanel[] components;
	private transient List<Image> windowIcons; // The form filler application icons
	...
	public FormDisplayer(Form tempF, JPanel[] tempComponents, User tempU, UserList tempUserList, GUI tempGUI, List<Image> tempIcons)
	{
	...
	private void prepareHashMaps()
	{
		...
		{
			timeToCompleteQuestions.put(p.getQuestionID(), Long.valueOf(0L));
			failedValidationChecks.put(p.getQuestionID(), Integer.valueOf(0));
		}
	}
	...
	private void prepareGUI()
	{
		...
		if (Toolkit.getDefaultToolkit().getScreenSize().getHeight() < 1000)
		{
			...
			this.setExtendedState(Frame.MAXIMIZED_BOTH); // Maximise it
		...
		this.setIconImages(windowIcons); // Set the icon to the form filler icons
	...
	private void preparePages()
	{
		...
		{
			JPanel currentComponentPage = new JPanel();
			
			currentComponentPage.setLayout(new BoxLayout(currentComponentPage, BoxLayout.PAGE_AXIS)); // Create a new vertical box layout
			currentComponentPage.setPreferredSize(new Dimension(1000,1000)); // Make the page arbitrarily large
			currentComponentPage.add(Box.createVerticalStrut(30)); // Add 30px vertical padding
			...
			for (int i = 0; i < QUESTIONS_PER_PAGE; i++)
			{
				...
				if (currentComponent < components.length)
				{
					...
					if (isAQuestion)
					{
						...
						currentComponentPage.add(Box.createVerticalGlue());
						...
						currentComponentPage.add(rowPanel);
						currentComponent++;
					}
					else // It's a header
					{
						...
						{
							currentComponentPage.add(Box.createVerticalGlue()); // Fills the vertical space
							currentComponentPage.add(component); // Add the header to the page
			...
			currentComponentPage.add(Box.createVerticalGlue()); // Fills the empty vertical space
			currentComponentPage.add(Box.createVerticalStrut(30)); // Add 30px vertical padding
			tempPages[pageNumber] = currentComponentPage; // Store the page in the pages array
			pageNumber++;
		}
