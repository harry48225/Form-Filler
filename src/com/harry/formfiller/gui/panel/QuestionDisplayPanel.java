...
public class QuestionDisplayPanel extends JPanel implements ActionListener, TableColumnModelListener, Helper
{
	...
	private void refreshTable()
	{
		...
		Question[] latestQuestionData = questions.getArray(); // Get the most recent question data
		...
		if (typeSort)
		{
			...
			latestQuestionData = questions.getArray();
		}
		if (difficultySort)
		{
		...
		if (difficultyFilter && typeFilter) // If they selected both filters we need to find the intersection of the filters
		{
			...
			latestQuestionData = new Question[nextIntersectionLocation];
			
			for (int i = 0; i < nextIntersectionLocation; i++)
			{
				latestQuestionData[i] = intersection[i];
			}
		}
		else if (difficultyFilter)
		{
			latestQuestionData = questions.filterByDifficulty(difficultySlider.getValue());
		}
		else if (typeFilter)
		{
			latestQuestionData = questions.filterByType(getTypeSelected());
		}
		
		populateTable(latestQuestionData); // Populate the table with the results of the sorts/filters
	}
